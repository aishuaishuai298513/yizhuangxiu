...
-(instancetype)init
{
    ...
    if(self)
    {
        ...
        __weak typeof (self)WeakSelkf = self;
        _paymentForm.completeHandle = ^(NSString *inputPwd)
        {
            ...
            {
                NSLog(@"密码是6为数字");
                if (WeakSelkf.getText) {
                    int ceshi =  WeakSelkf.getText(inputPwd);
                    
                    if (ceshi == 1) {
                        [WeakSelkf dismiss];
                        //[ITTPromptView showMessage:@"支付成功"];
                    }
                }
            }
            else
...
-(void)loadNib
{
    if(!_paymentForm)
    {
        ...
        NSLayoutConstraint *center_y = [NSLayoutConstraint constraintWithItem:_paymentForm attribute:NSLayoutAttributeCenterY relatedBy:NSLayoutRelationEqual toItem:self attribute:NSLayoutAttributeCenterY multiplier:1.0 constant:-50];
...
-(void)resetAlertFrame
{
    ...
    {
        CGRect alertFrame = _paymentForm.frame;
        alertFrame.origin.y = ([UIScreen mainScreen].bounds.size.height - alertFrame.size.height) / 2.0f-100;
